...
def test_basic_functionality():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "devonthink_schemas", "./app/schemas/devonthink_schemas.py"
        )
        ...
        request = DevonthinkSyncRequest(database_name="Reference", search_space_id=1)
        print(f"✅ Sync request created: {request.database_name}")

        # Test MCP client basics
        spec2 = importlib.util.spec_from_file_location(
            "devonthink_mcp_client", "./app/services/devonthink_mcp_client.py"
        )
