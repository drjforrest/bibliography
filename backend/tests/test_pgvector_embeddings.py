...
async def test_embedding_config():
    ...
    try:
        ...
        print(
            f"✅ Embedding Dimension: {getattr(config.embedding_model_instance, 'dimension', 'unknown')}"
        )
        ...
        embedding = await asyncio.to_thread(
            config.embedding_model_instance.embed, test_text
        )
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            if embeddings is not None and len(embeddings) == len(test_texts):
                ...
                print(
                    f"❌ Batch embedding failed: got {len(embeddings) if embeddings else 0} embeddings"
                )
                return False
...
async def test_new_sync_with_embeddings():
    ...
    response = input(
        "Would you like to test syncing a new document from DEVONthink with embeddings? [y/N]: "
    )
    if response.lower() != "y":
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            print(
                f"   Documents: {docs_before} -> {docs_after} (+{docs_after - docs_before})"
            )
            print(
                f"   Chunks: {chunks_before} -> {chunks_after} (+{chunks_after - chunks_before})"
            )
            ...
            if response.synced_count > 0:
                ...
                print(
                    f"   New docs with embeddings: {embedded_docs}/{len(recent_docs)}"
                )
