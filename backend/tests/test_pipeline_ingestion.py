...
async def test_pipeline_status():
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            print(
                f"   DEVONthink sync records: {sync_records} (synced: {synced_records})"
            )
            ...
            for i, paper in enumerate(recent_papers, 1):
                ...
                print(
                    f"      Has embedding: {'✅' if paper.document and paper.document.embedding else '❌'}"
                )
            ...
            if docs_with_embeddings > 0:
                ...
                if sample and sample.embedding:
                    print(
                        f"   Sample embedding dimensions: {len(sample.embedding) if hasattr(sample.embedding, '__len__') else 'Unknown'}"
                    )
                    print(
            ...
            if user and papers_count > 0:
                ...
                try:
                    ...
                    if success:
                        ...
                        print(
                            f"   Documents in FAISS: {new_stats.get('documents_indexed', 0)}"
                        )
                        ...
                        print(
                            f"   Search test results: {search_result.get('total_results', 0)} papers found"
                        )
            ...
            if papers_count > 0 and docs_with_embeddings > 0:
                ...
                print(
                    "   ⚠️  Papers exist but missing embeddings - check embedding service"
                )
            else:
