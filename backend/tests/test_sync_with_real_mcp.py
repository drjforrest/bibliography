...
async def test_sync_with_real_mcp():
    ...
    try:
        ...
        if not is_running:
            print(
                "   WARNING: DEVONthink is not running. Please start DEVONthink and try again."
            )
            return
        ...
        for item in root_content[:3]:  # Show first 3
            print(
                f"     - {item.get('name', 'Unknown')} ({item.get('recordType', 'Unknown')})"
            )
        ...
        if pdf_results:
            ...
            if uuid:
                ...
                if properties:
                    ...
                    print(
                        f"     - Creation Date: {properties.get('creationDate', 'Unknown')}"
                    )
                    print(
                        f"     - Word Count: {properties.get('wordCount', 'Unknown')}"
                    )
        ...
        recent_changes = await client.search_recent_changes(
            days=7, database_name="Reference"
        )
        ...
        print(
            "✅ All tests passed! The sync service is now connected to real DEVONthink data."
        )
