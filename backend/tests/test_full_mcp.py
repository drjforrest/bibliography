...
class MCPClient:
    ...
    async def send_initialize(self):
        ...
        init_request = {
            ...
            "params": {
                ...
                "clientInfo": {"name": "Bibliography Test Client", "version": "1.0.0"},
            },
        }
    ...
    async def send_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Invalid JSON response: {e}, stderr: {error_output.decode()}"
            )
...
async def test_devonthink_mcp():
    ...
    try:
        ...
        try:
            ...
            if (
                "result" in databases_response
                and "content" in databases_response["result"]
            ):
                    ...
                    for item in content:
                        if item.get("type") == "text":
                            try:
                                ...
                                print(
                                    f"  Found {len(db_data.get('databases', []))} open databases"
                                )
...
async def main():
    ...
    if not await test_mcp_availability():
        ...
        print(
            "💡 Make sure you're in a directory where npx can access mcp-server-devonthink"
        )
        sys.exit(1)
