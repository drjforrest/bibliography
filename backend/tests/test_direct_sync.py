...
async def test_direct_sync():
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            pdf_records = await mcp_client.search_records(
                "kind:pdf", database_name="Reference", limit=3
            )
                    ...
                    for record in pdf_records[:2]:  # Limit to 2 for testing
                        try:
                            ...
                            stats["details"].append(
                                f"Synced: {record.get('name', 'Unknown')}"
                            )
            ...
            if result["synced"] > 0:
                ...
                print(
                    "   💡 Issue is with get_open_databases MCP call, not the core pipeline."
                )
