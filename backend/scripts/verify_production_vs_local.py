...
async def check_database(label, database_url):
    ...
    print(f"{'=' * 60}")
    print(
        f"Connection: {database_url.split('@')[1] if '@' in database_url else 'REDACTED'}"
    )
...
async def main():
    ...
    # Check production via tunnel - use environment variables
    # Build connection string from environment variables
    prod_host = os.environ.get("DB_HOST", "localhost")
    prod_port = os.environ.get("DB_PORT", "5433")
    prod_user = os.environ.get("DB_USER", "postgres")
    prod_password = os.environ.get("DB_PASSWORD")
    prod_db = os.environ.get("DB_NAME", "hero_evidence_library_prod")

    if not prod_password:
        print(
            "✗ Error: DB_PASSWORD environment variable is required for production connection"
        )
        sys.exit(1)

    prod_url = (
        os.environ.get("DATABASE_URL")
        or f"postgresql+asyncpg://{prod_user}:{prod_password}@{prod_host}:{prod_port}/{prod_db}"
    )
