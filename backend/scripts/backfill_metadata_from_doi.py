...
from app.services.semantic_scholar_service import SemanticScholarService, GRAPH_API
...
class MetadataBackfillPipeline:
    ...
    def __init__(self, session_maker, dry_run: bool = False, force_overwrite: bool = True):
        self.session_maker = session_maker
        self.dry_run = dry_run
        self.force_overwrite = force_overwrite  # If True, overwrite existing fields with better data
    ...
    async def _fetch_from_semantic_scholar(self, doi: str) -> Optional[Dict]:
        ...
        try:
            ...
            fields = [
                "paperId", "title", "abstract", "year", "authors", "url", "citationCount",
                "venue", "volume", "issue", "pages", "fieldsOfStudy", "keywords", "isOpenAccess"
            ]
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                ...
                if data:
                    ...
                    if normalized:
                        logger.debug(f"✓ Found in Semantic Scholar: {len(normalized)} fields")
    ...
    def _normalize_semantic_scholar_data(self, s2_data: Dict) -> Dict:
        ...
        if s2_data.get("fieldsOfStudy"):
            subject_areas = [f for f in s2_data["fieldsOfStudy"] if f and isinstance(f, str)]
            if subject_areas:
    ...
    async def _enrich_paper_from_doi(
        ...
        try:
            ...
            if not merged_data:
                ...
                logger.warning(f"     DOI {doi} not found in Crossref or Semantic Scholar")
            ...
            if merged_data.get("title"):
                should_update = (
                    self.force_overwrite or 
                    not paper.title or 
                    len(merged_data["title"]) > len(paper.title)
                )
                if should_update:
                    if not self.dry_run:
            ...
            if merged_data.get("abstract"):
                should_update = (
                    ...
                    len(paper.abstract) < 50 or
                    (len(merged_data["abstract"]) > len(paper.abstract) + 100)  # Significantly better
                )
                if should_update:
                    if not self.dry_run:
            ...
            if merged_data.get("journal"):
                should_update = (
                    self.force_overwrite or
                    not paper.journal or 
                    len(paper.journal.strip()) == 0
                )
                if should_update:
                    if not self.dry_run:
            ...
            if merged_data.get("authors"):
                ...
                if isinstance(authors, list) and authors:
                    should_update = (
                        self.force_overwrite or
                        not paper.authors or 
                        len(paper.authors) == 0 or
                        len(authors) > len(paper.authors)  # More authors available
                    )
                    if should_update:
                        if not self.dry_run:
            ...
            if merged_data.get("keywords"):
                ...
                if isinstance(keywords, list) and keywords:
                    should_update = (
                        self.force_overwrite or
                        not paper.keywords or 
                        len(paper.keywords) == 0 or
                        len(keywords) > len(paper.keywords)  # More keywords available
                    )
                    if should_update:
                        if not self.dry_run:
            ...
            if merged_data.get("subject_areas"):
                ...
                if isinstance(subject_areas, list) and subject_areas:
                    should_update = (
                        self.force_overwrite or
                        not paper.subject_areas or 
                        len(paper.subject_areas) == 0
                    )
                    if should_update:
                        if not self.dry_run:
            ...
            if not paper.doi or paper.doi.strip() == "" or (self.force_overwrite and merged_data.get("doi")):
            ...
            if merged_data.get("citation_count") is not None:
                should_update = (
                    self.force_overwrite or
                    paper.citation_count is None or
            ...
            should_generate_lay_summary = (
                not paper.lay_summary or 
                len(paper.lay_summary.strip()) == 0 or
                (self.force_overwrite and merged_data.get("abstract"))  # Re-generate if we have better abstract
            )
            if should_generate_lay_summary and not self.dry_run:
                ...
                try:
                    ...
                    if lay_summary:
                        ...
                        self.llm_success += 1
                        logger.info(f"  ✓ Generated lay_summary ({len(lay_summary)} chars)")
                    else:
    ...
    async def enrich_all_papers(
        ...
        if self.force_overwrite:
            logger.info("⚡ FORCE OVERWRITE MODE - Will update existing fields with better data")
        ...
        for i, paper in enumerate(papers, 1):
            ...
            if not self.force_overwrite and self._already_enriched_with_doi(paper):
                ...
                logger.info(f"     Reason: Already enriched with DOI (use --no-overwrite=false to force)")
    ...
    def print_summary(self):
        ...
        if self.skipped_papers:
            ...
            logger.info(f"    - Already enriched: {len([p for p in self.skipped_papers if p['reason'] == 'Already enriched'])}")
        ...
        if self.successful_papers:
            ...
            for paper in self.successful_papers[:20]:  # Show first 20
                ...
                fields = ', '.join(paper['fields_updated']) if paper['fields_updated'] else 'none'
            ...
            if len(self.successful_papers) > 20:
                logger.info(f"  ... and {len(self.successful_papers) - 20} more successful papers")
        ...
        if self.failed_papers:
            ...
            if len(self.failed_papers) > 20:
                logger.info(f"  ... and {len(self.failed_papers) - 20} more failed papers")
        ...
        if self.skipped_papers:
            ...
            if len(self.skipped_papers) > 10:
                logger.info(f"  ... and {len(self.skipped_papers) - 10} more skipped papers")
