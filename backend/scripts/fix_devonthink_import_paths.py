...
async def find_papers_with_placeholder_paths(session: AsyncSession) -> List[ScientificPaper]:
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Fix devonthink_import placeholder paths")
    parser.add_argument(
        "--mode",
    ...
    # Get storage root
    storage_root = Path(config.PDF_STORAGE_ROOT or "./data/pdfs")
    ...
    search_paths.extend([p for p in common_paths if p.exists()])
