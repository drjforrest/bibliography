...
class LLMEnrichmentPipeline:
    ...
    async def enrich_all_papers(
        ...
        async with self.session_maker() as session:
            ...
            query = query.where(
                (ScientificPaper.abstract != None) | (ScientificPaper.full_text != None)
            )
    ...
    async def _enrich_paper(self, session: AsyncSession, paper_id: int):
        ...
        if not metadata.get("short_description"):
            ...
            if short_desc:
                ...
                logger.warning(
                    f"Short description generation returned None for paper {paper_id}"
                )
        ...
        if not metadata.get("insights"):
            ...
            if insights:
                ...
                logger.warning(
                    f"Insights generation returned None for paper {paper_id}"
                )
...
Summary:""",
    ...
    async def _generate_short_description(
        ...
        try:
            ...
            logger.info(
                f"Short desc LLM response: [{description}] ({len(description) if description else 0} chars)"
            )
            if description:
                ...
                if len(description) > 10:
                    ...
                    logger.warning(
                        f"Short description too short: {len(description)} chars"
                    )
                    return None
...
Format: ["Insight 1", "Insight 2", "Insight 3"]""",
            ...
            insights_text = await self._call_llm(
                messages, max_tokens=1200, temperature=0.7
            )
            logger.info(
                f"Insights LLM response: [{insights_text[:200] if insights_text else None}...] ({len(insights_text) if insights_text else 0} chars)"
            )
            if insights_text:
...
async def main():
    parser = argparse.ArgumentParser(
        description="Enrich papers with LLM-generated content"
    )
