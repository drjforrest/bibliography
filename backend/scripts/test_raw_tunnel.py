            ...
            try:
                ...
                if response:
                    ...
                    if b"PostgreSQL" in response:
                        version = response.split(b"\x00")[0].decode(
                            "utf-8", errors="ignore"
                        )
...
if test_port_connection("127.0.0.1", 5433):
    ...
    if test_postgres_protocol("127.0.0.1", 5433):
        print("  ✓ PostgreSQL protocol detected on port 5433")
    else:
        print("  ✗ PostgreSQL protocol NOT detected on port 5433")
else:
