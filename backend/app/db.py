...
class ScientificPaper(BaseModel, TimestampMixin):
    ...
    literature_type = Column(
        SQLAlchemyEnum(LiteratureType),
        nullable=False,
        default=LiteratureType.PEER_REVIEWED,
        index=True,
    )
    ...
    lay_summary = Column(
        Text, nullable=True
    )  # AI-generated lay summary for general audiences
    ...
    dt_source_uuid = Column(
        String(255), nullable=True, index=True
    )  # DEVONthink source UUID
...
class PaperAnnotation(BaseModel, TimestampMixin):
    ...
    # Relations
    paper_id = Column(
        Integer, ForeignKey("scientific_papers.id", ondelete="CASCADE"), nullable=False
    )
...
class Tag(BaseModel, TimestampMixin):
    ...
    papers = relationship(
        "ScientificPaper", secondary=paper_tags, back_populates="tag_objects"
    )
...
class NotificationType(str, Enum):
    MENTION_IN_ANNOTATION = "MENTION_IN_ANNOTATION"
    MENTION_IN_MESSAGE = "MENTION_IN_MESSAGE"
    NEW_ANNOTATION_ON_PAPER = "NEW_ANNOTATION_ON_PAPER"
...
class UserNotification(BaseModel, TimestampMixin):
    ...
    # Who triggered this notification (optional - can be null for system notifications)
    sender_id = Column(
        UUID(as_uuid=True),
    ...
    notification_type = Column(
        SQLAlchemyEnum(NotificationType), nullable=False, index=True
    )
    content = Column(Text, nullable=False)  # Preview text or message
...
class MessageTopic(BaseModel, TimestampMixin):
    __tablename__ = "message_topics"

    name = Column(String(100), nullable=False, index=True)
    icon = Column(String(50), nullable=True, default="chat")
    description = Column(Text, nullable=True)
    ...
    user = relationship("User", back_populates="message_topics")
...
class Message(BaseModel, TimestampMixin):
    ...
    # Topic relationship
    topic_id = Column(
        Integer, ForeignKey("message_topics.id", ondelete="CASCADE"), nullable=False
    )
    topic = relationship("MessageTopic", back_populates="messages")
...
class DevonthinkSync(BaseModel, TimestampMixin):
    ...
    # Sync tracking
    last_sync_date = Column(
        TIMESTAMP(timezone=True), nullable=True
    )  # Last sync to our system
    sync_status = Column(
        SQLAlchemyEnum(DevonthinkSyncStatus),
        nullable=False,
