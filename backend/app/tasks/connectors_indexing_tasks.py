    ...
    try:
        ...
        if connector.last_indexed_at:
            ...
            if last_indexed_naive > end_date:
                ...
                logger.info(
                    f"Using last_indexed_at ({start_date.strftime('%Y-%m-%d')}) as start date"
                )
        else:
            start_date = end_date - timedelta(
                days=30
            )  # Use 30 days instead of 365 to catch recent issues
            logger.info(
                f"No last_indexed_at found, using {start_date.strftime('%Y-%m-%d')} (30 days ago) as start date"
            )
        ...
        logger.info(
            f"Found {len(existing_docs_by_channel_id)} existing Slack documents in database"
        )
        ...
        for channel_name, channel_id in channels.items():
            try:
                ...
                try:
                    ...
                    channel_info = slack_client.client.conversations_info(
                        channel=channel_id
                    )
                    ...
                    if channel_info.get("channel", {}).get("is_private", False):
                        ...
                        is_member = channel_info.get("channel", {}).get(
                            "is_member", False
                        )
                        if not is_member:
                            logger.warning(
                    ...
                    if "not_in_channel" in str(e) or "channel_not_found" in str(e):
                        logger.warning(
                            f"Bot cannot access channel {channel_name} ({channel_id}). Skipping."
                        )
                ...
                # Generate summary
                summary_chain = (
                    SUMMARY_PROMPT_TEMPLATE | config.long_context_llm_instance
                )
                ...
                if existing_document:
                    ...
                    logger.info(
                        f"Updating existing document for channel {channel_name}"
                    )
                    ...
                    await session.execute(
                        delete(Chunk).where(Chunk.document_id == existing_document.id)
                    )
        ...
        logger.info(
            f"Slack indexing completed: {documents_indexed} new channels, {documents_updated} updated, {documents_skipped} skipped"
        )
...
async def index_notion_pages(
    ...
    try:
        ...
        logger.info(
            f"Found {len(existing_docs_by_page_id)} existing Notion documents in database"
        )
        ...
        logger.info(
            f"Notion indexing completed: {documents_indexed} new pages, {documents_updated} updated, {documents_skipped} skipped"
        )
        return total_processed, result_message
...
async def index_github_repos(
    ...
    try:
        ...
        existing_docs_lookup = {
            doc.document_metadata.get("full_path"): doc
            for doc in existing_docs
            if doc.document_metadata.get("full_path")
        }
        logger.info(
            f"Found {len(existing_docs_lookup)} existing GitHub documents in database for search space {search_space_id}"
        )
        ...
        for repo_full_name in repo_full_names_to_index:
            ...
            try:
                ...
                for file_info in files_to_index:
                    ...
                    if (
                        existing_doc
                        and existing_doc.document_metadata.get("sha") == file_sha
                    ):
...
async def index_linear_issues(
    ...
    try:
        ...
        logger.info(
            f"Found {len(existing_docs_by_issue_id)} existing Linear documents in database"
        )
        ...
        if existing_docs_by_issue_id:
            ...
            for idx, (issue_id, doc) in enumerate(
                list(existing_docs_by_issue_id.items())[:10]
            ):  # Log first 10
                logger.info(
                    f"  {idx+1}. {issue_id} - {doc.document_metadata.get('issue_identifier', 'Unknown')} - {doc.document_metadata.get('issue_title', 'Unknown')}"
                )
            if len(existing_docs_by_issue_id) > 10:
                logger.info(
                    f"  ...and {len(existing_docs_by_issue_id) - 10} more existing documents"
                )
        ...
        for issue in issues:
            try:
                ...
                if existing_document:
                    # Update existing document instead of creating a new one
                    logger.info(
                        f"Updating existing document for issue {issue_identifier} - {issue_title}"
                    )
