...
class MonitoringConfigRequest(BaseModel):
    ...
    folder_path: Optional[str] = Field(
        None, description="Specific folder path to monitor"
    )
    polling_interval_minutes: int = Field(
        30, description="Polling interval in minutes", ge=5, le=1440
    )
...
async def configure_monitoring(
    ...
    ingestion_service: AutomatedIngestionService = Depends(
        get_automated_ingestion_service
    ),
):
    """
    ...
    try:
        ...
        if config_request.polling_interval_minutes != 30:
            ingestion_service.polling_interval = (
                config_request.polling_interval_minutes * 60
            )
...
async def get_hierarchy_status(
    ...
    try:
        hierarchy_status = await ingestion_service.get_hierarchy_status(
            user_id=current_user.id, database_name=database_name
        )
...
async def force_full_sync(
    ...
    try:
        logger.info(
            f"Force full sync requested by user {current_user.id} for database {sync_request.database_name}"
        )
        ...
        if sync_request.folder_path is None:
            ...
            return DevonthinkSyncResponse(
                ...
                details=[
                    ...
                    "Check monitoring status for progress",
                ],
            )
        else:
