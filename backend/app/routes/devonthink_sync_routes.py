...
async def sync_devonthink_database_local(
    ...
    try:
        ...
        if request.folder_path is None:  # Full database sync
            background_tasks.add_task(_run_sync_in_background, request, USER_ID)
            return DevonthinkSyncResponse(
                ...
                details=[
                    "Background sync initiated for entire database",
                    f"Using local user ID: {USER_ID}",
                ],
            )
        else:
...
async def sync_devonthink_database(
    ...
    try:
        ...
        # For large syncs, run in background
        if request.folder_path is None:  # Full database sync
            background_tasks.add_task(sync_service.sync_database, request, user.id)
            return DevonthinkSyncResponse(
                success=True,
...
async def get_folder_hierarchy(
    ...
    try:
        ...
        stmt = (
            ...
            .where(
                DevonthinkFolder.user_id == user.id, DevonthinkFolder.depth_level == 0
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get folder hierarchy: {str(e)}"
        )
...
async def monitor_devonthink_changes(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to monitor changes: {str(e)}"
        )
    ...
    try:
        if not search_space_id:
            ...
            if not search_space:
                raise HTTPException(
                    status_code=400,
                    detail="No search space found. Please specify search_space_id.",
                )
        ...
        if changes["total_changes"] <= 10:
            ...
            background_tasks.add_task(sync_service.sync_database, sync_request, user.id)
            await sync_service.close()
            return DevonthinkSyncResponse(
                success=True,
                message=f"Incremental sync started in background. {changes['total_changes']} changes detected.",
                details=[
                    f"Processing {changes['new_records']} new and {changes['updated_records']} updated records"
                ],
            )

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Incremental sync failed: {str(e)}"
        )
...
async def get_sync_stats(
    ...
    try:
        ...
        stmt = (
            select(
                DevonthinkSync.sync_status, func.count(DevonthinkSync.id).label("count")
            )
            .where(DevonthinkSync.user_id == user.id)
...
async def _build_folder_hierarchy(
    session: AsyncSession, folder: DevonthinkFolder, user_id: UUID
) -> DevonthinkFolderHierarchy:
