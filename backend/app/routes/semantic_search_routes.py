...
async def find_similar_papers(
    ...
    try:
        similar_papers = await semantic_service.similar_papers_search(
            paper_id=paper_id, user_id=str(user.id), limit=limit
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Similar papers search failed: {str(e)}"
        )
...
async def get_search_suggestions(
    ...
    try:
        ...
        return SearchSuggestionsResponse(
            query=query, suggestions=suggestions, total_suggestions=len(suggestions)
        )
