...
class SearchQuery(BaseModel):
    ...
    search_space_id: Optional[int] = Field(
        None, description="Optional search space filter"
    )
...
async def ask_question(
    ...
    try:
        logger.info(
            f"Question from user {current_user.id}: {question_query.question[:50]}..."
        )
...
async def rebuild_vector_index(
    ...
    try:
        ...
        success = await rag_service.build_vector_store_from_papers(
            user_id=str(current_user.id), search_space_id=search_space_id
        )

        if success:
            stats = rag_service.get_stats()
            return {"message": "Vector index rebuilt successfully", "stats": stats}
        else:
            raise HTTPException(
...
async def get_rag_stats(
    ...
    try:
        ...
        return StatsResponse(status="error", error=str(e), documents_indexed=0)
