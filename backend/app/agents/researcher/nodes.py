...
async def fetch_relevant_documents(
        ...
        for connector in connectors_to_search:
            ...
            try:
                ...
                if streaming_service and writer:
                    friendly_name = get_connector_friendly_name(connector)
                    streaming_service.only_update_terminal(
                        f"⚠️ Error searching {friendly_name}: {str(e)}", "error"
                    )
                    writer({"yeild_value": streaming_service._format_annotations()})
    ...
    if streaming_service and writer:
        streaming_service.only_update_terminal(
            f"📚 Collected {len(deduplicated_sources)} unique sources across all connectors"
        )
        writer({"yeild_value": streaming_service._format_annotations()})
...
async def process_sections(
    ...
    if not answer_outline:
        streaming_service.only_update_terminal(
            "❌ Error: No answer outline was provided. Cannot generate report.", "error"
        )
        writer({"yeild_value": streaming_service._format_annotations()})
        return {
    ...
    streaming_service.only_update_terminal(
        f"✨ Starting to draft {len(answer_outline.answer_outline)} sections using {len(relevant_documents)} relevant document chunks"
    )
    writer({"yeild_value": streaming_service._format_annotations()})
    ...
    return {"final_written_report": final_written_report}
...
async def process_section_with_documents(
    ...
    try:
        ...
        if not documents_to_use:
            ...
            if state and state.streaming_service and writer:
                state.streaming_service.only_update_terminal(
                    f'⚠️ Warning: No relevant documents found for section: "{section_title}"',
                    "warning",
                )
                writer({"yeild_value": state.streaming_service._format_annotations()})
        ...
        async with async_session_maker() as db_session:
            ...
            sub_state = {"db_session": db_session, "chat_history": state.chat_history}
                    ...
                    if new_content and new_content != complete_content:
                        ...
                        if delta and state and state.streaming_service and writer:
                            ...
                            for i in range(len(section_contents)):
                                if (
                                    i in section_contents
                                    and section_contents[i]["content"]
                                ):
                                    # Add section header
                                    ...
                                    # Add section content
                                    content_lines = section_contents[i][
                                        "content"
                                    ].split("\n")
                            ...
                            writer(
                                {
                                    "yeild_value": state.streaming_service._format_annotations()
                                }
                            )
        ...
        if state and state.streaming_service and writer:
            state.streaming_service.only_update_terminal(
                f'❌ Error processing section "{section_title}": {str(e)}', "error"
            )
            writer({"yeild_value": state.streaming_service._format_annotations()})
