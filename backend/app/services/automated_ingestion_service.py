...
class AutomatedIngestionService:
    ...
    async def start_monitoring(self):
        ...
        logger.info(
            f"Starting automated ingestion monitoring (interval: {self.polling_interval}s)"
        )
    ...
    async def _monitoring_cycle(self):
        ...
        logger.info(
            f"Starting monitoring cycle for {len(self.monitored_databases)} databases"
        )
        ...
        async with get_async_session_context() as session:
            ...
            try:
                ...
                if config["last_check"]:
                    hours_since_check = (
                        datetime.now(timezone.utc) - config["last_check"]
                    ).total_seconds() / 3600
                    hours_to_check = max(1, int(hours_since_check) + 1)  # Add buffer

                changes = await sync_service.monitor_changes(
                    database_name, days=hours_to_check / 24
                )

                # Check if there are any new or updated records
                total_changes = len(changes.get("new_records", [])) + len(
                    changes.get("updated_records", [])
                )

                if total_changes > 0:
                    logger.info(
                        f"Found {total_changes} changes in {monitor_key}, starting incremental sync"
                    )
    ...
    async def _incremental_sync(
        self, session: AsyncSession, config: Dict, changes: Dict
    ):
        """Perform incremental sync for changed records."""
        user_id = config["user_id"]
        database_name = config["database_name"]
        ...
        if new_records:
            ...
            for record in new_records:
                try:
                    ...
                    logger.error(
                        f"Failed to sync new record {record.get('uuid', 'Unknown')}: {str(e)}"
                    )
        ...
        if updated_records:
            ...
            for record in updated_records:
                try:
                    ...
                    logger.debug(
                        f"Re-synced updated record: {record.get('name', 'Unknown')}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to re-sync updated record {record.get('uuid', 'Unknown')}: {str(e)}"
                    )
        ...
        if new_records or updated_records:
            try:
                logger.info(
                    "Rebuilding Enhanced RAG vector store after incremental sync"
                )
                rebuilt_success = (
                    await sync_service.enhanced_rag.build_vector_store_from_papers(
                ...
                if rebuilt_success:
                    ...
                    logger.warning(
                        "Failed to rebuild FAISS vector store after incremental sync"
                    )
            except Exception as e:
    ...
    async def force_full_sync(
        ...
        """
        logger.info(
            f"Starting forced full sync for user {user_id}, database {database_name}"
        )
        ...
        async with get_async_session_context() as session:
            ...
            try:
                ...
                if monitor_key in self.monitored_databases:
                    self.monitored_databases[monitor_key]["last_check"] = datetime.now(
                        timezone.utc
                    )
    ...
    async def get_hierarchy_status(
        self, user_id: UUID, database_name: str = "Reference"
    ) -> Dict:
        """
        ...
        async with get_async_session_context() as session:
            ...
            stmt = (
                select(DevonthinkFolder)
                .where(DevonthinkFolder.user_id == user_id)
            ...
            for folder in folders:
                if folder.parent_dt_uuid == parent_uuid:
                    folder_node = {
                        ...
                        "depth": folder.depth_level,
                        "sync_status": (
                            folder.sync_status.value
                            if folder.sync_status
                            else "unknown"
                        ),
                        "last_sync": (
                            folder.last_sync_date.isoformat()
                            if folder.last_sync_date
                            else None
                        ),
