...
class DevonthinkMCPClientRealV2:
    ...
    async def _read_response(self) -> Dict[str, Any]:
        ...
        try:
                ...
                while True:
                    try:
                        chunk = await asyncio.wait_for(
                            self.process.stdout.read(65536), timeout=10.0
                        )
                        if not chunk:
                            break
            ...
            logger.error(
                f"Failed to decode MCP response (length: {len(response_text) if 'response_text' in locals() else 'unknown'})"
            )
            logger.error(
                f"Response preview: {response_text[:500] if 'response_text' in locals() else 'N/A'}..."
            )
                ...
                if isinstance(content_str, str):
                    try:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Error with UTF-8 encoding fallback: {str(e2)}"
                            )
                            return None
                else:
