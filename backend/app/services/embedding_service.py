...
class EmbeddingService:
        ...
        try:
            ...
            if hasattr(self.embedding_model, "embed"):
                ...
                embedding = await asyncio.to_thread(
                    self.embedding_model.embed, text.strip()
                )
            elif hasattr(self.embedding_model, "embed_query"):
                # LangChain OpenAI embeddings
                embedding = await asyncio.to_thread(
                    self.embedding_model.embed_query, text.strip()
                )
            else:
                raise ValueError(
                    f"Unknown embedding model interface: {type(self.embedding_model)}"
                )
        ...
        try:
            ...
            if hasattr(self.embedding_model, "embed_batch"):
                ...
                embeddings = await asyncio.to_thread(
                    self.embedding_model.embed_batch, valid_texts
                )
            elif hasattr(self.embedding_model, "embed_documents"):
                # LangChain embeddings
                embeddings = await asyncio.to_thread(
                    self.embedding_model.embed_documents, valid_texts
                )
            else:
    ...
    async def embed_document(
        self, document_id: int, force_update: bool = False
    ) -> bool:
        """
    ...
    async def embed_document_chunks(
        ...
        try:
            ...
            if not chunks_to_embed:
                logger.debug(
                    f"All chunks for document {document_id} already have embeddings"
                )
                return True
            ...
            if len(embeddings) != len(chunks_to_embed):
                logger.error(
                    f"Mismatch in embeddings count: got {len(embeddings)}, expected {len(chunks_to_embed)}"
                )
                return False
            ...
            logger.info(
                f"Successfully embedded {len(chunks_to_embed)} chunks for document {document_id}"
            )
            return True
    ...
    async def create_and_embed_chunks(self, document: Document) -> bool:
        ...
        try:
            ...
            if len(embeddings) != len(chunk_texts):
                logger.error(
                    f"Mismatch in chunk embeddings: got {len(embeddings)}, expected {len(chunk_texts)}"
                )
            ...
            for i, (chunk_text, embedding) in enumerate(zip(chunk_texts, embeddings)):
                if embedding:  # Only create chunk if we have a valid embedding
                    chunk = Chunk(
                        content=chunk_text, embedding=embedding, document_id=document.id
                    )
            ...
            logger.info(
                f"Created and embedded {chunks_created} chunks for document {document.id}"
            )
            return chunks_created > 0

        except Exception as e:
            logger.error(
                f"Error creating and embedding chunks for document {document.id}: {str(e)}"
            )
            await self.session.rollback()
            return False
                ...
                try:
                    ...
                    if not existing_chunks:
                        ...
                        if await self.create_and_embed_chunks(document):
                            ...
                            new_chunks_stmt = select(Chunk).where(
                                Chunk.document_id == document.id
                            )
                            new_chunks_result = await self.session.execute(
                                new_chunks_stmt
                            )
                        ...
                        if await self.embed_document_chunks(document.id):
                            chunks_embedded = len(
                                [c for c in existing_chunks if not c.embedding]
                            )
    ...
    def get_embedding_stats(self) -> dict:
        ...
        return {
            ...
            "embedding_dimension": getattr(
                config.embedding_model_instance, "dimension", "unknown"
            ),
