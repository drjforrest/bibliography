...
class SemanticSearchService:
        ...
        try:
            ...
            if search_type == "semantic":
                results = await self._semantic_search(
                    query, user_id, search_space_id, limit
                )
            ...
            if min_confidence > 0.0:
                results = [
                    r
                    for r in results
                    if r.get("paper_confidence", 0.0) >= min_confidence
                ]

            # Enhance results with paper-specific information
            enhanced_results = await self._enhance_paper_results(
                results, include_abstracts
            )
    ...
    async def _semantic_search(
        self, query: str, user_id: str, search_space_id: Optional[int], limit: int
    ) -> List[Dict]:
    ...
    async def _enhance_paper_results(
        self, results: List[Dict], include_abstracts: bool
    ) -> List[Dict]:
        ...
        for result in results:
            ...
            paper_stmt = select(ScientificPaper).where(
                ScientificPaper.document_id == document_id
            )
    ...
    async def _generate_search_insights(
        ...
        avg_confidence = (
            sum(p.get("confidence_score", 0.0) for p in papers) / total_papers
            if total_papers > 0
            else 0.0
        )
        ...
        publication_years = sorted(
            year_counts.items(), key=lambda x: x[0], reverse=True
        )[:10]
        ...
        try:
            ...
            if document.content and not document.embedding:
                try:
                    ...
                    logger.info(
                        f"Generated embedding for document {document.id} using HuggingFace model"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to generate embedding for document {document.id}: {str(e)}"
                    )
            ...
            logger.error(
                f"Error processing document {document.id} for search: {str(e)}"
            )
            raise
        ...
        try:
            ...
            while start < len(content):
                ...
                if chunk_content:
                    ...
                    try:
                        chunk_embedding = config.embedding_model_instance.embed_query(
                            chunk_content
                        )
                    except Exception as e:
                        logger.warning(
                            f"Failed to generate embedding for chunk: {str(e)}"
                        )
    ...
    async def _create_document_chunks_simple(self, document: Document):
        ...
        try:
            ...
            await self.session.flush()
            logger.info(
                f"Created {len(chunks)} chunks for document {document.id} (without embeddings)"
            )

        except Exception as e:
            logger.error(
                f"Error creating simple chunks for document {document.id}: {str(e)}"
            )
            raise
    ...
    async def _create_document_chunks_with_working_embeddings(self, document: Document):
        ...
        try:
            ...
            logger.error(
                f"Error creating chunks with embeddings for document {document.id}: {str(e)}"
            )
            raise

    async def similar_papers_search(
        self, paper_id: int, user_id: str, limit: int = 5
    ) -> List[Dict]:
        """
        ...
        paper_stmt = (
            ...
            .options(joinedload(ScientificPaper.document))
        ...
        if not search_text.strip():
            search_text = paper.document.content[
                :500
        ...
        for result in results:
            ...
            enhanced_result = await self._enhance_paper_results(
                [result], include_abstracts=True
            )
            if enhanced_result:
    ...
    async def get_search_suggestions(
        ...
        # Search in keywords
        keyword_stmt = (
            select(ScientificPaper.keywords)
            .join(Document, ScientificPaper.document_id == Document.id)
            .join(SearchSpace, Document.search_space_id == SearchSpace.id)
            .where(
                *base_conditions,
                ScientificPaper.keywords.op("&&")(func.array([f"%{partial_query}%"])),
            )
            .limit(limit * 2)
        )
