    ...
    async def _sync_records(
        ...
        try:
            ...
            if folder_path:
                ...
                pdf_records = [
                    r for r in pdf_records if r.get("path", "").startswith(folder_path)
                ]
            else:
                pdf_records = await self.mcp_client.search_records(
                    search_query, database_name=database_name
        ...
        try:
            ...
            if os.path.isdir(tmp_path):
                ...
                if pdf_files:
                    ...
                    raise ValueError(
                        f"No PDF files found in DEVONthink export directory: {tmp_path}"
                    )
            ...
            if (
                not os.path.exists(actual_pdf_path)
                or os.path.getsize(actual_pdf_path) == 0
            ):
            ...
            logger.info(
                f"Copied PDF {dt_uuid} to {relative_path} (size: {os.path.getsize(actual_pdf_path)} bytes)"
            )
