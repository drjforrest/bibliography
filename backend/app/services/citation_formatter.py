            ...
            if len(paper.authors) == 1:
                ...
                for i in range(19):
                    formatted_authors.append(
                        CitationFormatter._format_author_apa(paper.authors[i])
                    )
                formatted_authors.append("...")
                formatted_authors.append(
                    CitationFormatter._format_author_apa(paper.authors[-1])
                )
                citation_parts.append(f"{', '.join(formatted_authors)}.")
        ...
        if paper.authors:
            if len(paper.authors) == 1:
                ...
                if len(first_author_parts) >= 2:
                    first_author = (
                        f"{first_author_parts[-1]}, {' '.join(first_author_parts[:-1])}"
                    )
                else:
    ...
    def _format_chicago(paper: ScientificPaper) -> str:
        ...
        if paper.authors:
            if len(paper.authors) == 1:
                ...
                authors_str = (
                    ", ".join(paper.authors[:-1]) + f", and {paper.authors[-1]}"
                )
    ...
    def _format_ieee(paper: ScientificPaper) -> str:
        ...
        if paper.authors:
            if len(paper.authors) <= 6:
                ...
                if len(first_author_parts) >= 2:
                    initials = (
                        ". ".join([part[0] for part in first_author_parts[:-1]]) + "."
                    )
