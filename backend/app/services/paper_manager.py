...
class PaperManagerService:
    ...
    async def process_pdf_file(
        ...
        try:
            ...
            if extracted_data.get("processing_status") == "failed":
                ...
                return {
                    "status": "failed",
                    "error": extracted_data.get("extraction_metadata", {}).get(
                        "error", "Unknown error"
                    ),
                    "file_path": file_path,
                }
    ...
    async def _create_paper_records(
        self,
        extracted_data: Dict,
        stored_path: str,
        file_uuid: str,
        ...
        scientific_paper = ScientificPaper(
            ...
            extraction_metadata=extracted_data.get("extraction_metadata", {}),
            confidence_score=extracted_data.get("extraction_metadata", {}).get(
    ...
    async def get_storage_stats(self) -> Dict:
        ...
        # Papers by status
        status_stmt = select(
            ScientificPaper.processing_status, func.count(ScientificPaper.id)
    ...
    def start_folder_watcher(self, user_id: str, search_space_id: int):
        ...
        logger.info(
            f"Started folder watcher for user {user_id}, search_space {search_space_id}"
        )
