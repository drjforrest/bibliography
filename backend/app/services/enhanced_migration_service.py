...
from app.db import DevonthinkSync, DevonthinkSyncStatus, SearchSpace
...
class EnhancedMigrationService:
    ...
    async def start_complete_migration(
        self,
        database_name: str,
        user_id: UUID,
        search_space_id: int,
        ...
        job_id = (
            f"migration_{database_name}_{user_id}_{int(datetime.now().timestamp())}"
        )
    ...
    async def _execute_migration_pipeline(self, config: MigrationJobConfig):
        ...
        try:
            ...
            await self.progress_tracker.update_phase(
                job_id, MigrationPhase.INITIALIZING
            )
            ...
            if not await self._validate_prerequisites(config):
                ...
                await self.progress_tracker.fail_job(
                    job_id, "Prerequisites validation failed"
                )
                return
            logger.info("   ✅ Prerequisites validated successfully")

            # Phase 2: Map directories
            logger.info("🗺️ Phase 2: Mapping directory structure...")
            await self.progress_tracker.update_phase(
            ...
            await self.progress_tracker.update_phase(
                job_id,
                MigrationPhase.MAPPING_DIRECTORIES,
                {"directories_mapped": folder_count},
            )
            logger.info(f"   ✅ Mapped {folder_count} directories")

            # Phase 3: Discover records
            logger.info("🔍 Phase 3: Discovering records to migrate...")
            await self.progress_tracker.update_phase(
                job_id, MigrationPhase.DISCOVERING_RECORDS
            )
            ...
            if not config.force_resync:
                logger.info("   🔍 Filtering out already synced records...")
                record_uuids = await self._filter_pending_records(record_uuids, user_id)
                logger.info(
                    f"   ✅ {len(record_uuids)} records remaining after filtering"
                )

            await self.progress_tracker.set_total_records(
                job_id, len(record_uuids), record_uuids
            )

            if not record_uuids:
                logger.info("ℹ️  No records to process - migration completed")
                await self.progress_tracker.complete_job(job_id)
                return

            # Phase 4: Migrate records
            logger.info(
                f"📦 Phase 4: Migrating {len(record_uuids)} records (PDF processing, chunking, vector embedding)..."
            )
            await self.progress_tracker.update_phase(
                job_id, MigrationPhase.MIGRATING_RECORDS
            )
            await self._migrate_records_with_tracking(config, record_uuids)
            logger.info("✅ Record migration phase completed")

            # Phase 5: Generate lay summaries
            logger.info("🧠 Phase 5: Generating AI lay summaries...")
            # Phase 5: Lay summaries are already generated by LLMEnrichmentService (using LM Studio)
            # The separate Ollama lay summary service is redundant, so we skip this phase
            logger.info(
                "⏭️  Skipping redundant lay summary phase (already generated by LLMEnrichmentService)"
            )
            await self.progress_tracker.update_phase(
                job_id,
                MigrationPhase.GENERATING_LAY_SUMMARIES,
                {
                    "skipped_reason": "Already handled by LLMEnrichmentService (LM Studio)"
                },
            )
            logger.info("✅ Lay summary generation phase skipped (already complete)")
    ...
    async def _validate_prerequisites(self, config: MigrationJobConfig) -> bool:
        ...
        try:
            ...
            search_space = await self._get_search_space(
                config.search_space_id, UUID(config.user_id)
            )
            if not search_space:
            ...
            open_db_names = [db.get("name") for db in databases]
            logger.info(
                f"Open DEVONthink databases: {', '.join(open_db_names) if open_db_names else 'None'}"
            )

            if not any(db.get("name") == config.database_name for db in databases):
                logger.error(
                    f"Database '{config.database_name}' not found in open databases"
                )
                logger.error(
                    f"Please open the '{config.database_name}' database in DEVONthink and try again"
                )
                return False
    ...
    async def _map_directories(self, config: MigrationJobConfig) -> int:
        ...
        try:
            hierarchy = await self.sync_service._map_directory_hierarchy(
                config.database_name, UUID(config.user_id), config.folder_path
            )
    ...
    async def _discover_records(self, config: MigrationJobConfig) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Searching for PDFs in database: {config.database_name}")
            # Request enough to get all PDFs (user has ~1309, request 2000 to be safe)
            pdf_records = await self.sync_service.mcp_client.search_records(
                search_query, database_name=config.database_name, limit=2000
            )
            ...
            if config.folder_path:
                ...
                logger.info(
                    f"After folder filter ({config.folder_path}): {len(valid_pdf_records)} records"
                )

            return [record["uuid"] for record in valid_pdf_records if "uuid" in record]

        except Exception as e:
            logger.error(f"Record discovery failed: {str(e)}")
            raise

    async def _filter_pending_records(
        self, record_uuids: List[str], user_id: UUID
    ) -> List[str]:
        """Filter out records that are already successfully synced or user-deleted"""
        try:
            # Query for records that are already synced (multiple checks for robustness)
            # Check 1: Records marked as SYNCED
            stmt_synced = select(DevonthinkSync.dt_uuid).where(
            ...
            result_synced = await self.session.execute(stmt_synced)
            synced_uuids = {row[0] for row in result_synced}

            # Check 2: Records that are user-deleted (should not be re-synced)
            stmt_deleted = select(DevonthinkSync.dt_uuid).where(
                DevonthinkSync.user_id == user_id,
                DevonthinkSync.user_deleted == True,  # noqa: E712
                DevonthinkSync.dt_uuid.in_(record_uuids),
            )
            result_deleted = await self.session.execute(stmt_deleted)
            deleted_uuids = {row[0] for row in result_deleted}

            if deleted_uuids:
                logger.info(
                    f"Filtering out {len(deleted_uuids)} user-deleted records from sync"
                )

            # Check 3: Records that have a scientific_paper_id (even if status isn't SYNCED)
            # This catches records that were partially synced or have papers already
            stmt_with_paper = select(DevonthinkSync.dt_uuid).where(
                DevonthinkSync.user_id == user_id,
                DevonthinkSync.dt_uuid.in_(record_uuids),
                DevonthinkSync.scientific_paper_id.isnot(None),
            )
            result_with_paper = await self.session.execute(stmt_with_paper)
            synced_uuids.update({row[0] for row in result_with_paper})

            # Combine all records to skip (synced + deleted)
            all_skipped_uuids = synced_uuids | deleted_uuids

            # Return only unsynced and non-deleted records
            pending = [uuid for uuid in record_uuids if uuid not in all_skipped_uuids]
            skipped_count = len(record_uuids) - len(pending)

            if skipped_count > 0:
                deleted_count = len(deleted_uuids)
                synced_count = len(synced_uuids) - deleted_count  # Don't double-count
                logger.info(
                    f"✅ Filtered {len(record_uuids)} records: "
                    f"{synced_count} already synced, {deleted_count} user-deleted (skipped), "
                    f"{len(pending)} pending"
                )
            else:
                logger.info(
                    f"📋 All {len(record_uuids)} records are new - none to skip"
                )
            ...
            logger.warning(
                "Processing all records due to filtering error (may re-sync existing records)"
            )
            # If filtering fails, process all records
            return record_uuids

    async def _migrate_records_with_tracking(
        self, config: MigrationJobConfig, record_uuids: List[str]
    ):
        ...
        logger.info(
            f"🚀 Starting migration of {total_records} records for job {job_id}"
        )
        ...
        for i in range(0, len(record_uuids), batch_size):
            ...
            logger.info(
                f"📦 Processing batch {batch_start}-{batch_end} of {total_records} records"
            )
            ...
            for j, record_uuid in enumerate(batch):
                ...
                try:
                    # Mark as processing
                    await self.progress_tracker.mark_record_processing(
                        job_id, record_uuid
                    )

                    # Get record details for logging
                    record_props = (
                        await self.sync_service.mcp_client.get_record_properties(
                    ...
                    if not record_props:
                        logger.error(
                            f"❌ Record {current_record_num}/{total_records} ({record_uuid[:8]}...): Could not retrieve properties"
                        )
                    ...
                    logger.info(
                        f"📄 Processing record {current_record_num}/{total_records}: '{record_name}' ({record_uuid[:8]}...)"
                    )
                    logger.debug(f"   📁 Path: {record_path}")

                    # Perform the actual sync (PDF will be read from file path, not via MCP get_record_content)
                    ...
                    if sync_result:
                        logger.info(
                            f"✅ Completed record {current_record_num}/{total_records}: '{record_name}'"
                        )
                        # Mark as completed
                        await self.progress_tracker.mark_record_completed(
                            job_id, record_uuid
                        )
                    else:
                        logger.error(
                            f"❌ Failed record {current_record_num}/{total_records}: '{record_name}' - Sync returned False"
                        )
                        await self.progress_tracker.mark_record_failed(
                            job_id, record_uuid, "Sync operation returned False"
                        )

                except Exception as e:
                    error_msg = f"Failed to sync record {record_uuid}: {str(e)}"
                    record_name = (
                        record_props.get("name", "Unknown")
                        if "record_props" in locals()
                        else "Unknown"
                    )
                    logger.error(
                        f"❌ Record {current_record_num}/{total_records} '{record_name}' failed: {str(e)}"
                    )
                    await self.progress_tracker.mark_record_failed(
                        job_id, record_uuid, error_msg
                    )
            ...
            logger.info(
                f"📦 Completed batch {batch_start}-{batch_end}. Progress: {processed_count}/{total_records} ({(processed_count / total_records * 100):.1f}%)"
            )

            # Small delay between batches to prevent overwhelming the system
            await asyncio.sleep(0.1)

    async def _sync_single_record_with_logging(
        self,
        record: Dict,
        database_name: str,
        user_id: UUID,
        ...
        try:
            logger.debug(
                f"   🔄 [{current_num}/{total_num}] Starting sync for '{record_name}'"
            )

            # Step 1: Download and process PDF
            logger.debug(
                f"   💾 [{current_num}/{total_num}] Downloading PDF content..."
            )
            ...
            stmt = select(DevonthinkSync).where(
                DevonthinkSync.dt_uuid == record_uuid, DevonthinkSync.user_id == user_id
            )
            ...
            if existing_sync and not force_resync:
                ...
                if existing_sync.user_deleted:
                    logger.info(
                        f"   🚫 [{current_num}/{total_num}] Skipping '{record_name}' - deleted by user"
                    )
                    return True
                ...
                if existing_sync.sync_status == DevonthinkSyncStatus.SYNCED:
                    logger.debug(
                        f"   ⏭️  [{current_num}/{total_num}] Skipping '{record_name}' - already synced"
                    )
                    return True

            # Step 2: Process and store the record (delegate to original method)
            # Note: PDF content is retrieved via file path in _copy_pdf_binary, not via MCP get_record_content
            # (MCP's get_record_content returns text, not binary PDF, so we skip that step)
            logger.debug(
                f"   🔄 [{current_num}/{total_num}] Processing and storing record..."
            )
            ...
            if sync_successful:
                logger.debug(
                    f"   💾 [{current_num}/{total_num}] Stored file and metadata to database"
                )

                # Step 4: Text chunking (this happens inside the sync method)
                logger.debug(
                    f"   ✂️  [{current_num}/{total_num}] Text chunking completed"
                )

                # Step 5: Vector embedding and storage (this also happens inside)
                logger.debug(
                    f"   🧠 [{current_num}/{total_num}] Vector embeddings generated and stored to pgvector"
                )

                # Step 6: PostgreSQL metadata update
                logger.debug(
                    f"   💾 [{current_num}/{total_num}] Metadata updated in PostgreSQL"
                )

                return True
            else:
                logger.error(
                    f"   ❌ [{current_num}/{total_num}] Sync operation failed for '{record_name}'"
                )
                return False

        except Exception as e:
            logger.error(
                f"   ❌ [{current_num}/{total_num}] Exception during sync of '{record_name}': {str(e)}"
            )
            return False
    ...
    async def retry_failed_records(self, job_id: str) -> bool:
        ...
        try:
            ...
            await self._migrate_records_with_tracking(
                config, list(failed_records.keys())
            )

            logger.info(
                f"Retried {len(failed_records)} failed records for job {job_id}"
            )
            return True
    ...
    async def _generate_lay_summaries_for_migrated_records(
        self, config: MigrationJobConfig
    ):
        ...
        try:
            ...
            if not await self.lay_summary_service.test_connection():
                logger.warning(
                    f"⚠️  Ollama not available for lay summary generation in job {job_id}"
                )
                await self.progress_tracker.update_phase(
                    job_id,
                    MigrationPhase.GENERATING_LAY_SUMMARIES,
            ...
            logger.debug("   🔍 Retrieving completed records from Redis...")
            ...
            if not completed_records:
                logger.info(
                    f"ℹ️  No completed records found for lay summary generation in job {job_id}"
                )
                return

            total_records = len(completed_records)
            logger.info(
                f"📊 Generating lay summaries for {total_records} completed records"
            )
            ...
            for i in range(0, len(completed_records), batch_size):
                ...
                logger.info(
                    f"🧠 Processing lay summary batch {batch_start}-{batch_end} of {total_records}"
                )
                ...
                if not progress or progress.phase == MigrationPhase.PAUSED:
                    logger.info(
                        f"⏸️  Migration job {job_id} paused during lay summary generation"
                    )
                    return

                # Get paper data for this batch
                logger.debug("   📄 Retrieving paper data for batch...")
                papers_data = await self._get_papers_for_summary_generation(
                    batch, user_id
                )

                if not papers_data:
                    logger.warning(
                        f"   ⚠️  No papers found for batch {batch_start}-{batch_end}"
                    )
                    continue

                logger.debug(
                    f"   ✅ Found {len(papers_data)} papers for lay summary generation"
                )

                # Generate summaries for the batch
                logger.debug(
                    "   🧠 Generating lay summaries using Ollama (max 2 concurrent)..."
                )
                batch_results = await self.lay_summary_service.generate_batch_summaries(
                    papers_data,
                    max_concurrent=2,  # Limit concurrency for Ollama
                )

                logger.info(
                    f"   ✅ Generated {len(batch_results)} lay summaries in batch"
                )
                ...
                for paper_id, summary in batch_results.items():
                    ...
                    paper_data = next(
                        (p for p in papers_data if p["id"] == paper_id), None
                    )
                    paper_title = (
                        paper_data["title"][:50] + "..."
                        if paper_data and len(paper_data["title"]) > 50
                        else (paper_data["title"] if paper_data else "Unknown")
                    )

                    try:
                        logger.debug(
                            f"   💾 [{processed_summaries}/{total_records}] Updating lay summary for '{paper_title}'"
                        )
                        await self._update_paper_with_lay_summary(paper_id, summary)
                        summaries_generated += 1
                        logger.debug(
                            f"   ✅ [{processed_summaries}/{total_records}] Lay summary updated ({len(summary)} chars)"
                        )
                    except Exception as e:
                        summaries_failed += 1
                        logger.error(
                            f"   ❌ [{processed_summaries}/{total_records}] Failed to update lay summary for '{paper_title}': {str(e)}"
                        )

                # Log batch progress
                progress_pct = (
                    (processed_summaries / total_records * 100)
                    if total_records > 0
                    else 0
                )
                logger.info(
                    f"📊 Lay summary progress: {processed_summaries}/{total_records} ({progress_pct:.1f}%) - {summaries_generated} successful, {summaries_failed} failed"
                )
            ...
            success_rate = (
                (summaries_generated / total_records * 100) if total_records > 0 else 0
            )
            logger.info(
                f"🎉 Lay summary generation completed! Generated {summaries_generated}/{total_records} summaries ({success_rate:.1f}% success rate)"
            )
            ...
            logger.error(
                f"❌ Error during lay summary generation for job {job_id}: {str(e)}"
            )
            # Don't fail the entire migration for summary generation issues

    async def _get_papers_for_summary_generation(
        self, record_uuids: List[str], user_id: UUID
    ) -> List[Dict]:
                    ...
                    papers_data.append(
                        {
                            ...
                            "full_text": (
                                paper.full_text[:5000] if paper.full_text else None
                            ),  # Limit text size
                        }
                    )
    ...
    async def _update_paper_with_lay_summary(self, paper_id: int, lay_summary: str):
        ...
        try:
            ...
            if paper:
                ...
                logger.debug(
                    f"Updated lay summary for paper {paper_id}: {len(lay_summary)} characters"
                )
            else:
