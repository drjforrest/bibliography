...
class EnhancedMigrationService:
    ...
    async def _filter_pending_records(
        ...
        try:
            ...
            logger.info(
                f"Filtered {len(record_uuids)} records to {len(pending)} pending"
            )
            return pending
    ...
    async def _sync_single_record_with_logging(
        ...
        try:
            ...
            logger.debug(
                f"   📁 [{current_num}/{total_num}] Retrieving file content from DEVONthink..."
            )

            try:
                file_content = await self.sync_service.mcp_client.get_record_content(
                    record_uuid
                )
                if file_content:
                    logger.debug(
                        f"   ✅ [{current_num}/{total_num}] Retrieved {len(file_content)} bytes of content"
                    )
                else:
                    logger.warning(
                        f"   ⚠️  [{current_num}/{total_num}] No content retrieved for '{record_name}'"
                    )
            except Exception as e:
                logger.error(
                    f"   ❌ [{current_num}/{total_num}] Failed to get content: {str(e)}"
                )
                return False
