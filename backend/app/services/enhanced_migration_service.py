...
class EnhancedMigrationService:
    ...
    async def _execute_migration_pipeline(self, config: MigrationJobConfig):
        ...
        try:
            ...
            logger.info(f"⏭️  Skipping redundant lay summary phase (already generated by LLMEnrichmentService)")
            await self.progress_tracker.update_phase(
                job_id, MigrationPhase.GENERATING_LAY_SUMMARIES,
                {"skipped_reason": "Already handled by LLMEnrichmentService (LM Studio)"}
            )
            logger.info(f"✅ Lay summary generation phase skipped (already complete)")
    ...
    async def _validate_prerequisites(self, config: MigrationJobConfig) -> bool:
        ...
        try:
            ...
            logger.info(f"Open DEVONthink databases: {', '.join(open_db_names) if open_db_names else 'None'}")
            
            if not any(db.get("name") == config.database_name for db in databases):
                logger.error(f"Database '{config.database_name}' not found in open databases")
                logger.error(f"Please open the '{config.database_name}' database in DEVONthink and try again")
                return False
    ...
    async def _discover_records(self, config: MigrationJobConfig) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Filtered to {len(valid_pdf_records)} valid PDF records (excluded {len(pdf_records) - len(valid_pdf_records)} annotations/non-PDFs)")
            ...
            if config.folder_path:
                ...
                logger.info(f"After folder filter ({config.folder_path}): {len(valid_pdf_records)} records")
    ...
    async def _filter_pending_records(
        ...
        try:
            ...
            # Check 2: Records that have a scientific_paper_id (even if status isn't SYNCED)
            ...
            if skipped_count > 0:
                logger.info(
                    f"✅ Filtered {len(record_uuids)} records: {skipped_count} already in library (skipped), {len(pending)} pending"
                )
            else:
                logger.info(
            ...
            logger.warning("Processing all records due to filtering error (may re-sync existing records)")
    ...
    async def _generate_lay_summaries_for_migrated_records(
        ...
        try:
            ...
            for i in range(0, len(completed_records), batch_size):
                ...
                logger.debug(
                    f"   🧠 Generating lay summaries using Ollama (max 2 concurrent)..."
                )
