...
class EnhancedRAGService:
    ...
    async def build_vector_store_from_papers(
        self, user_id: str, search_space_id: Optional[int] = None
    ) -> bool:
        """
    ...
    async def semantic_search(
        ...
        try:
            ...
            if not self.vectorstore:
                success = await self.build_vector_store_from_papers(
    ...
    async def ask_question(
        self, question: str, user_id: str, search_space_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_search_insights(
        ...
        avg_similarity = (
            sum(r.get("similarity_score", 0.0) for r in results) / total_papers
        )
        ...
        # Publication years
        years = [
            r.get("publication_year") for r in results if r.get("publication_year")
        ]
        year_counts = {}
        for year in years:
