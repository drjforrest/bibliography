...
class DashboardService:
    ...
    async def _get_user_basic_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_papers_per_space": (
                round(total_papers / total_spaces, 2) if total_spaces > 0 else 0
            ),
        }
    ...
    async def _get_paper_analytics(self, user_id: str) -> Dict[str, Any]:
        ...
        year_distribution_stmt = (
            ...
            .order_by(desc(ScientificPaper.publication_year))
            .limit(10)
        )

        year_result = await self.session.execute(year_distribution_stmt)
        year_distribution = [
            {"year": year, "count": count} for year, count in year_result.fetchall()
        ]

        # Top journals
        journal_distribution_stmt = (
            select(
                ScientificPaper.journal, func.count(ScientificPaper.id).label("count")
            )
            .join(Document, ScientificPaper.document_id == Document.id)
            .join(SearchSpace, Document.search_space_id == SearchSpace.id)
    ...
    async def _get_quality_metrics(self, user_id: str) -> Dict[str, Any]:
        ...
        doi_percentage = (
            (papers_with_doi / total_papers * 100) if total_papers > 0 else 0
        )
    ...
    async def _get_system_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_annotations": total_annotations,
            "avg_papers_per_user": (
                round(total_papers / total_users, 2) if total_users > 0 else 0
            ),
        }
    ...
    async def _get_global_user_activity(self) -> Dict[str, Any]:
        ...
        active_users_papers_stmt = (
            select(func.count(func.distinct(SearchSpace.user_id)))
            .join(Document, SearchSpace.id == Document.search_space_id)
            .join(ScientificPaper, Document.id == ScientificPaper.document_id)
    ...
    async def _get_global_content_analytics(self) -> Dict[str, Any]:
        ...
        popular_journals_stmt = (
            select(
                ScientificPaper.journal, func.count(ScientificPaper.id).label("count")
            )
            .where(ScientificPaper.journal.isnot(None))
            .group_by(ScientificPaper.journal)
            .order_by(desc(func.count(ScientificPaper.id)))
            .limit(10)
        )
        ...
        return {"popular_journals": popular_journals}
    ...
    async def get_recent_annotation_activity(
        self, current_user_id: str, limit: int = 20
    ) -> List[Dict[str, Any]]:
        """
        Get recent annotation activity from all users (excluding current user).

        Args:
            current_user_id: Current user ID (to exclude their own annotations)
        ...
        for annotation, user, paper in result.fetchall():
            activities.append(
                {
                    "id": annotation.id,
                    "type": "annotation",
                    ...
                    "content": annotation.content[:200],  # Preview only
