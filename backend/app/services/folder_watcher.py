...
class PDFFileHandler(FileSystemEventHandler):
    ...
    async def _process_after_delay(self, file_path: str, delay: float = 2.0):
        ...
        try:
            ...
            if size1 != size2:
                logger.info(
                    f"File still being written, delaying processing: {file_path}"
                )
...
class FolderWatcherService:
            ...
            if existing_pdfs:
                ...
                for pdf_path in existing_pdfs:
                    try:
                        ...
                        logger.error(
                            f"Error processing existing file {pdf_path}: {str(e)}"
                        )
            else:
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "pdf_count": (
                len(list(self.watched_folder.glob("*.pdf")))
                if self.watched_folder.exists()
                else 0
            ),
        }
