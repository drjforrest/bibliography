...
class LaySummaryService:
    ...
    async def generate_lay_summary(
        ...
        try:
            ...
            if summary:
                ...
                logger.debug(
                    f"Generated lay summary for '{title[:50]}...': {len(cleaned_summary)} characters"
                )
    ...
    def _prepare_input_text(
        self, title: str, abstract: Optional[str], full_text: Optional[str]
    ) -> str:
...
Lay Summary:"""
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            async with session.post(
                ...
                if success:
                    logger.info(
                        f"✅ Ollama connection successful (model: {self.model})"
                    )
                else:
                    logger.error(
                        f"❌ Ollama connection failed with status {response.status}"
                    )
                return success
    ...
    async def generate_batch_summaries(
        self, papers_data: list, max_concurrent: int = 3
    ) -> Dict[str, str]:
        """
        ...
        logger.info(
            f"Generated lay summaries for {len(results)}/{len(papers_data)} papers"
        )
        return results
