...
class MigrationProgressTracker:
    ...
    async def create_migration_job(self, config: MigrationJobConfig) -> str:
        ...
        await self.redis_client.hset(
            progress_key, mapping={"data": json.dumps(asdict(progress), default=str)}
        )
    ...
    async def update_phase(
        self, job_id: str, phase: MigrationPhase, metadata: Dict[str, Any] = None
    ):
    ...
    async def _update_counts(self, job_id: str):
        ...
        if progress:
            ...
            if elapsed_minutes > 0:
                progress.records_per_minute = (
                    progress.processed_records / elapsed_minutes
                )

                remaining_records = progress.total_records - progress.processed_records
                if progress.records_per_minute > 0:
                    progress.estimated_minutes_remaining = int(
                        remaining_records / progress.records_per_minute
                    )
    ...
    async def get_progress(self, job_id: str) -> Optional[MigrationProgress]:
        ...
        if data:
            ...
            progress_dict["started_at"] = datetime.fromisoformat(
                progress_dict["started_at"]
            )
            progress_dict["updated_at"] = datetime.fromisoformat(
                progress_dict["updated_at"]
            )
            if progress_dict.get("estimated_completion"):
                progress_dict["estimated_completion"] = datetime.fromisoformat(
                    progress_dict["estimated_completion"]
                )
    ...
    async def _save_progress(self, progress: MigrationProgress):
        ...
        await self.redis_client.hset(
            progress_key, "data", json.dumps(asdict(progress), default=str)
        )
    ...
    async def get_pending_records(self, job_id: str) -> Set[str]:
        ...
        completed_records = await self.redis_client.smembers(
    ...
    async def resume_job(self, job_id: str) -> bool:
        ...
        if progress.phase in [MigrationPhase.PAUSED, MigrationPhase.FAILED]:
            progress.phase = (
                MigrationPhase.MIGRATING_RECORDS
            )  # Resume at migration phase
            progress.updated_at = datetime.now(timezone.utc)
            await self._save_progress(progress)
    ...
    async def pause_job(self, job_id: str):
        ...
        progress = await self.get_progress(job_id)
        if progress and progress.phase not in [
