...
class AnnotationService:
    ...
    async def create_annotation(
        self, paper_id: int, user_id: str, annotation_data: AnnotationCreate
    ) -> PaperAnnotation:
        """
        ...
        logger.info(
            f"Created annotation {annotation.id} for paper {paper_id} by user {user_id}"
        )
        return annotation

    async def get_annotation(
        self, annotation_id: int, user_id: str
        ...
        stmt = (
            ...
            .options(joinedload(PaperAnnotation.user))
            .where(
                and_(
    ...
    async def get_paper_annotations(
        self, paper_id: int, user_id: str, include_private: bool = True
    ...
    async def get_user_annotations(
        ...
        stmt = (
            select(PaperAnnotation)
            .options(
                joinedload(PaperAnnotation.paper), joinedload(PaperAnnotation.user)
            )
        ...
        stmt = (
            stmt.order_by(PaperAnnotation.created_at.desc()).limit(limit).offset(offset)
        )
    ...
    async def update_annotation(
        self, annotation_id: int, user_id: str, update_data: AnnotationUpdate
    ) -> Optional[PaperAnnotation]:
        """
        ...
        stmt = select(PaperAnnotation).where(
            and_(
                PaperAnnotation.id == annotation_id, PaperAnnotation.user_id == user_id
            )
        )
    ...
    async def get_annotation_stats(
        ...
        # Annotations by type
        type_stmt = select(
            PaperAnnotation.annotation_type, func.count(PaperAnnotation.id)
        ...
        # Privacy breakdown
        privacy_stmt = select(
            PaperAnnotation.is_private, func.count(PaperAnnotation.id)
    ...
    async def toggle_annotation_privacy(
        ...
        logger.info(
            f"Toggled privacy for annotation {annotation_id} by user {user_id} to {'private' if annotation.is_private else 'public'}"
        )
        return annotation
