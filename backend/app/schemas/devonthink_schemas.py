...
class DevonthinkRecordProperties(DevonthinkRecordBase):
    ...
    custom_meta_data: Optional[Dict[str, Any]] = Field(
...
class DevonthinkFolderHierarchy(BaseModel):
    ...
    children: List["DevonthinkFolderHierarchy"] = Field(
...
class DevonthinkSyncRequest(BaseModel):
    ...
    database_name: str = Field(
        default="Reference", description="DEVONthink database name"
    )
    folder_path: Optional[str] = Field(
        None, description="Specific folder to sync (optional)"
    )
...
class MCPToolResponse(BaseModel):
    ...
    result: Dict[str, Any] = Field(
        default_factory=dict, description="Tool response data"
    )
