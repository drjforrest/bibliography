...
class BibliographyAPI:
    ...
    def __init__(
        self, base_url: str, auth_base_url: str = None, token: Optional[str] = None
    ):
    ...
    def get_papers(
        ...
        response = requests.get(
            f"{self.base_url}/papers/", params=params, headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_room_stats(self) -> Dict:
        ...
        response = requests.get(
            f"{self.base_url}/papers/stats/by-room", headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def search_papers(self, query: str, search_space_id: Optional[int] = None) -> Dict:
        ...
        response = requests.post(
            f"{self.base_url}/papers/search", json=data, headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def upload_paper(
        self, file, search_space_id: int, literature_type: str = "PEER_REVIEWED"
    ) -> Dict:
        """Upload a PDF paper."""
        ...
        response = requests.post(
            f"{self.base_url}/papers/upload", files=files, data=data, headers=headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_search_spaces(self) -> Dict:
        ...
        response = requests.get(f"{self.base_url}/search-spaces/", headers=self.headers)
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def semantic_search(
        ...
        response = requests.post(
            f"{self.base_url}/semantic-search/", json=data, headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_user_dashboard(self) -> Dict:
        ...
        response = requests.get(f"{self.base_url}/dashboard/user", headers=self.headers)
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_global_dashboard(self) -> Dict:
        ...
        response = requests.get(
            f"{self.base_url}/dashboard/global", headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_dashboard_overview(self) -> Dict:
        ...
        response = requests.get(
            f"{self.base_url}/dashboard/overview", headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_devonthink_stats(self) -> Dict:
        ...
        response = requests.get(
            "http://localhost:8000/devonthink/stats", headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_devonthink_folders(self) -> Dict:
        ...
        response = requests.get(
            "http://localhost:8000/devonthink/folders", headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_devonthink_sync_status(self) -> Dict:
        ...
        response = requests.get(
            "http://localhost:8000/devonthink/sync/status", headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_user_stats(self) -> Dict:
        ...
        response = requests.get(
            f"{self.base_url}/admin/users/stats", headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def get_paper_pdf(self, paper_id: int) -> bytes:
        ...
        response = requests.get(
            f"{self.base_url}/papers/{paper_id}/pdf", headers=self.headers
        )
    ...
    def get_annotations(self, paper_id: int) -> Dict:
        ...
        response = requests.get(
            f"{self.base_url}/annotations/paper/{paper_id}", headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def add_annotation(
        ...
        response = requests.post(
            f"{self.base_url}/annotations/", json=data, headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def rag_ask_question(self, question: str, search_space_id: int = None) -> Dict:
        ...
        response = requests.post(
            f"{self.base_url}/enhanced-rag/ask", json=data, headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def rag_search(
        self, query: str, search_space_id: int = None, limit: int = 10
    ) -> Dict:
        """Enhanced RAG search."""
        ...
        response = requests.post(
            f"{self.base_url}/enhanced-rag/search", json=data, headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
    ...
    def send_chat_message(self, message: str, search_space_id: int = None) -> Dict:
        ...
        response = requests.post(
            "http://localhost:8000/api/v1/chats/chat", json=data, headers=self.headers
        )
        return (
            response.json() if response.status_code == 200 else {"error": response.text}
        )
...
def show_lobby(api: BibliographyAPI):
    ...
    with col1:
        ...
        st.markdown("---")
        st.markdown(
            "Access your curated collection of peer-reviewed scientific articles, research papers, and academic publications."
        )
        if st.button(
            "📖 Enter",
            key="enter_peer_reviewed",
            type="primary",
    ...
    with col2:
        ...
        st.markdown("---")
        st.markdown(
            "Browse technical reports, policy documents, white papers, and other grey literature sources."
        )
        if st.button(
            "📖 Enter", key="enter_grey_lit", type="primary", use_container_width=True
        ):
    ...
    with col3:
        ...
        if st.button(
            "📖 Coming Soon", key="enter_news", disabled=True, use_container_width=True
        ):
            pass
...
def show_room_interface(api: BibliographyAPI):
    ...
    search_spaces = (
        search_spaces_result.get("search_spaces", [])
        if "error" not in search_spaces_result
        else []
    )

    # Main content tabs
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["📚 Library", "🔍 Search", "💬 Chat", "📊 Dashboard", "🔧 Admin"]
    )
...
def library_browser_tab(api: BibliographyAPI):
    ...
    if st.session_state.selected_room:
        ...
        st.info(
            f"Filtering: {room_labels.get(st.session_state.selected_room, st.session_state.selected_room)}"
        )
    ...
    with col_tree:
        ...
        if "error" in folders_result:
            ...
            st.info(
                "No folders synced yet. Go to Admin → DEVONthink Sync to sync your library."
            )
...
def render_folder_tree(folder: Dict, api: BibliographyAPI, level: int = 0):
    ...
    with st.expander(
        ...
        expanded=(folder_key in st.session_state.expanded_folders),
    ):
...
def display_paper_with_pdf_and_annotations(paper: Dict, api: BibliographyAPI):
    ...
    pdf_tab, annotations_tab, metadata_tab = st.tabs(
        ["📖 PDF", "📝 Annotations", "ℹ️ Metadata"]
    )
    ...
    with st.form(key=f"new_annotation_{paper_id}"):
        ...
        with col1:
            annotation_content = st.text_area(
                "Note", placeholder="Enter your annotation...", height=100
            )
        with col2:
            page_number = st.number_input("Page", min_value=1, value=1, step=1)
            is_private = st.checkbox(
                "Private", value=True, help="Only you can see private annotations"
            )
            ...
            for ann in annotations:
                with st.container():
                    ...
                    with col1:
                        ...
                        st.markdown(
                            f"{privacy_icon} **Page {ann.get('page_number', 'N/A')}**"
                        )
...
def search_tab(api: BibliographyAPI, search_spaces: List[Dict]):
    ...
    search_type = st.radio(
        "Search Type", ["Keyword Search", "Semantic Search"], horizontal=True
    )
...
def keyword_search_panel(api: BibliographyAPI, search_spaces: List[Dict]):
    ...
    with col1:
        query = st.text_input(
            "Search query", placeholder="Enter keywords, title, author, etc."
        )
    ...
    with col2:
        ...
        selected_space = st.selectbox(
            "Search in", options=list(search_space_options.keys()), index=0
        )
...
def browse_papers_tab(api: BibliographyAPI, search_spaces: List[Dict]):
    ...
    with col1:
        ...
        search_space_options["All Spaces"] = None
        selected_space = st.selectbox(
            "Search Space", options=list(search_space_options.keys()), index=0
        )
    ...
    literature_type = (
        st.session_state.selected_room if st.session_state.selected_room else None
    )
    papers_result = api.get_papers(
        search_space_id=search_space_id, literature_type=literature_type, limit=limit
    )
...
def upload_papers_tab(api: BibliographyAPI, search_spaces: List[Dict]):
    ...
    with st.form("upload_form"):
        ...
        with col1:
            selected_space = st.selectbox(
                "Upload to Search Space", options=list(search_space_options.keys())
            )
        ...
        with col2:
            literature_type = st.selectbox(
                ...
                index=["PEER_REVIEWED", "GREY_LITERATURE", "NEWS"].index(
                    default_lit_type
                ),
        ...
        if submit and uploaded_file:
            ...
            with st.spinner("Processing PDF... This may take a few moments."):
                result = api.upload_paper(
                    uploaded_file, search_space_id, literature_type
                )
            ...
            if "error" in result:
                ...
                st.warning(
                    f"Upload completed with status: {result.get('status', 'unknown')}"
                )
    ...
    with col2:
        ...
        if st.button("📋 Get Citation", key=f"cite_{paper['id']}"):
            ...
            selected_style = st.selectbox(
                "Citation Style", options=citation_styles, key=f"style_{paper['id']}"
            )
...
def semantic_search_tab(api: BibliographyAPI, search_spaces: List[Dict]):
    ...
    if st.button("🧠 Semantic Search") and query:
        ...
        with col4:
            if insights.get("top_journals"):
                ...
                st.metric(
                    "Top Journal",
                    (
                        f"{top_journal['journal'][:20]}..."
                        if len(top_journal["journal"]) > 20
                        else top_journal["journal"]
                    ),
                )
        ...
        for i, result in enumerate(results):
            ...
            with st.expander(
                f"📄 {paper_info.get('title', 'Untitled')} (Score: {result.get('score', 0):.3f})"
            ):
                col1, col2 = st.columns([3, 1])
                ...
                with col2:
                    ...
                    if paper_info.get("confidence_score"):
                        st.metric(
                            ...
                            f"{paper_info['confidence_score']:.1%}",
                        )
                    ...
                    if st.button("🔍 Find Similar", key=f"similar_{paper_info['id']}"):
                        ...
                        if "error" not in similar_result:
                            ...
                            for sim_paper in similar_result.get("similar_papers", []):
                                ...
                                if sim_info:
                                    st.write(
                                        f"- **{sim_info.get('title', 'Untitled')}** (Score: {sim_paper.get('score', 0):.3f})"
                                    )
    ...
    if st.button("🔄 Load Full Dashboard"):
        ...
        if recent_activity:
            for activity in recent_activity[:5]:  # Show last 5 activities
                ...
                time_ago = (
                    datetime.now().replace(tzinfo=activity_time.tzinfo) - activity_time
                )
...
def admin_tab(api: BibliographyAPI):
    ...
    admin_tab1, admin_tab2, admin_tab3 = st.tabs(
        ["📚 DEVONthink Sync", "👥 User Management", "📊 System Stats"]
    )
...
def devonthink_sync_panel(api: BibliographyAPI):
    ...
    with col2:
        database_name = st.text_input(
            "Database Name", value="Reference", key="dt_db_name"
        )
                ...
                [
                    {
                        ...
                        "Path": (
                            s.get("dt_path", "")[:50] + "..."
                            if len(s.get("dt_path", "")) > 50
                            else s.get("dt_path", "")
                        ),
                        "Last Sync": (
                            s.get("last_synced_at", "")[:19]
...
def system_stats_panel(api: BibliographyAPI):
    ...
    if st.button("🌐 Load Global Dashboard", key="load_global_dash"):
        ...
        with col1:
            st.metric(
                "Total Storage", f"{storage_metrics.get('total_size_gb', 0):.2f} GB"
            )
        with col2:
            st.metric(
                "Average File Size",
                f"{storage_metrics.get('avg_file_size_mb', 0):.2f} MB",
            )
        with col3:
            processing_stats = global_result.get("processing_stats", {})
            st.metric(
                "Global Confidence",
                f"{processing_stats.get('global_avg_confidence', 0):.1%}",
            )
